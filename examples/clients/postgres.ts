...
import {
  ClientInterface,
  StatementRunOptions,
} from "@thequinndev/query-manager/manager/client.interface";
import {
  query,
  queryGroup,
  queryParameter,
...
const pgClient = (client: pg.Client): ClientInterface<pg.Client> => {
  ...
  const statementRun = async (options: StatementRunOptions) => {
    ...
    const result = resolvedParams
      ? client.query(queryItem.query, resolvedParams)
      : client.query(queryItem.query);
...
const userQueryManager = QueryManager({
  ...
  queries: queryGroup([
    query({
      ...
      description:
        "Insert single row via function for test. Returns the new ID.",
    ...
    query({
      ...
      parameters: [queryParameter("id", userTableSchema.shape.id)],
...
try {
  ...
  // Insert
  const id1 = await userQueryManager.run(
    "insertViaFunctionWithRetrievalHandler",
    {
      name: "test",
      description: "test",
    },
  ...
  const id2 = await userQueryManager.run(
    "insertViaFunctionWithQueryTypeProvided",
    {
      name: "test",
      description: "test",
    },
  ...
  const selectIndividualColumns = await userQueryManager.run(
    "selectIndividualColumns",
    {
      id: 1,
    },
  );
  ...
  const updateUserDescription = await userQueryManager.run(
  ...
  console.log(error);
  console.log(
    "If you continue to get unexpected errors, run pnpm pg:down then pnpm pg:up to reset the database.",
  );
} finally {
