...
describe("QueryManager", () => {
  const userQueryManager = QueryManager({
    ...
    queries: queryGroup([
      ...
      query({
        ...
        query:
          "insert into user_table (name, description, date) values (?, ?, ?)",
        parameters: [
      ...
      query({
        ...
        query:
          "insert or replace into user_table (id, name, description, date) values (?, ?, ?, ?)",
        parameters: [
  ...
  it("should then fail to select the delete id", async () => {
    await expect(
      userQueryManager.run("selectSingleTest", {
        id: id as number,
      }),
    ).rejects.toThrow();
  });
  ...
  it("Will allow custom running of operations", async () => {
    const result = await userQueryManager.doOperationAsClient(
      "selectIndividualColumns",
      ({ client, queryItem, parameters, result }) => {
        return result(
          client.prepare(queryItem.query).get(
            queryItem.parameters?.arrayResolver(
              parameters({
                id: 2,
              }),
            ),
